    ...
    return (
        <aside className="[grid-area:aside] px-5 flex flex-col  h-95 overflow-hidden overflow-y-auto gap-4 py-4 w-65">
            <div className='text-center'>
                ...
                <span className='cursor-pointer'>Limpiar filtros</span>
            </div>
            {
                filters.map(filter => (
                                ...
                                <Disclosure.Panel className="flex flex-col gap-2">
                                    {
                                        filter.options.map(option => (
                                            <label key={option} className="flex items-center gap-4 text-white/80 cursor-pointer hover:text-white">
                                                <input type="radio" name={filter.id} value={option.toLowerCase()} className="accent-[#00B4C8]" />
                                                {option}
                                            </label>
                                        ))}
                                </Disclosure.Panel>
